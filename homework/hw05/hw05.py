...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, guess):
        nonlocal past_attempts
        nonlocal balance

        if len(past_attempts) == 3:
            return f"Too many incorrect attempts. Attempts: {past_attempts}"
...
def merge(incr_a, incr_b):
    ...
    yield next_a if next_a is not None else next_b
