...
class VendingMachine:
    ...
    def restock(self, quantity):
        self.stock += quantity
        return f"Current {self.snack} stock: {self.stock}"
    ...
    def vend(self):
        ...
        if self.funds > 0:
            change = f" and ${self.funds} change"
            self.funds = 0
        else: change = ''
        return f"Here is your {self.snack}" + change + "."
...
class Coin:
    ...
    def worth(self):
        bonus_worth = max((Mint.current_year - self.year) - 50, 0)

        return self.cents + bonus_worth
...
def is_bst(t):
    ...
    if t.is_leaf():
        ...
        return bst_max(left) <= t.label  or t.label < bst_min(right) and is_bst(left)
    return False
...
def all_labels(t):
    if t.is_leaf():
        ...
        return sum([t.label] + [all_labels(b) for b in t.branches], [])
