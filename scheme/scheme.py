...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if isinstance(operator, MacroProcedure):
            foo = operator.apply_macro(expr.rest, env)
            return scheme_eval(foo, env)
        else:
            arguments = expr.rest.map(lambda a: scheme_eval(a, env))
...
def eval_all(expressions, env):
    ...
    """
    # BEGIN PROBLEM 7
    while expressions and expressions.rest:
    ...
    # curr_result = None
    # while expressions:
    #     curr_result = scheme_eval(expressions.first, env, tail)
    #     expressions = expressions.rest
    # return curr_result
    # END PROBLEM 7
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if isinstance(formals, Pair) or isinstance(vals, Pair):
            raise SchemeError("Not enough or too many vals given")
        return new_frame
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError("A problem with BuiltInProcedure.apply has appeared!")
        # END PROBLEM 3
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        bound_result = scheme_eval(expressions.rest.first, env)
        env.define(variable, bound_result)
        return variable
        # END PROBLEM 5
    elif isinstance(target, Pair) and scheme_symbolp(target.first):
        # BEGIN PROBLEM 9
        variable = target.first
        lambda_expr = Pair('lambda', Pair(target.rest, expressions.rest))
        evaled_lambda_expr = scheme_eval(lambda_expr, env)
        env.define(variable, evaled_lambda_expr)
        return variable
...
def do_and_form(expressions, env):
    ...
    while expressions.rest:
        ...
        if curr_val is False and is_false_primitive(curr_val):
            return False
        expressions = expressions.rest
...
def do_or_form(expressions, env):
    ...
    # if result or is_true_primitive(result):
    #     return result
    ...
    while expressions.rest:
        curr_val = scheme_eval(expressions.first, env)
        if curr_val or is_true_primitive(curr_val):
            return curr_val
        expressions = expressions.rest
...
def make_let_frame(bindings, env):
    ...
    while bindings:
        ...
        names = Pair(curr_binding.first, names)
        values = Pair(scheme_eval(curr_binding.rest.first, env), values)
...
def do_define_macro(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 9
        "*** YOUR CODE HERE ***"
        var_name = target.first
        formals = target.rest
        body = expressions.rest
        validate_formals(formals)
        env.define(var_name, MacroProcedure(formals, body, env))
        return var_name
        # END PROBLEM 9
    else:
        bad_target = target.first if isinstance(target, Pair) else target
...
def optimize_tail_calls(prior_eval_function):
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        while isinstance(result, Thunk):
            result = prior_eval_function(result.expr, result.env, tail)
        return result
        # END
